...
//class HoEStudy : public edm::one::EDAnalyzer<>  {
class HoEStudy : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit HoEStudy(const edm::ParameterSet&);
  ~HoEStudy();
...
//
HoEStudy::HoEStudy(const edm::ParameterSet& iConfig) :
...
void
HoEStudy::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
//  using mynamespace::HoEStudy;
  n_pho = 0;
  pho_eb.clear();
  pho_ee.clear();
  pho_gap_eb_ee.clear();
...
int HoEStudy::calDIPhi(int iPhi1, int iPhi2) {
...
int HoEStudy::calDIEta(int iEta1, int iEta2) {
...
float HoEStudy::getMinEnergyHCAL(HcalDetId id) const {
...
void HoEStudy::reallocate_setaddress(int n_pho_, int n_hcalhit_)
{
...
void
HoEStudy::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
