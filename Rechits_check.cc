...
//class Rechits_check : public edm::one::EDAnalyzer<>  {
class Rechits_check : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit Rechits_check(const edm::ParameterSet&);
  ~Rechits_check();
...
//
Rechits_check::Rechits_check(const edm::ParameterSet& iConfig) :
...
void
Rechits_check::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
//  using mynamespace::Rechits_check;
  n_pho = 0;
  pho_eb.clear();
  pho_ee.clear();
  pho_gap_eb_ee.clear();
...
int Rechits_check::calDIPhi(int iPhi1, int iPhi2) {
...
int Rechits_check::calDIEta(int iEta1, int iEta2) {
...
float Rechits_check::getMinEnergyHCAL(HcalDetId id) const {
...
void Rechits_check::reallocate_setaddress(int n_pho_, int n_hcalhit_)
{
...
void
Rechits_check::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(Rechits_check);
