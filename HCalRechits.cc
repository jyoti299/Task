...
class HCalRechits : public edm::one::EDAnalyzer<edm::one::SharedResources>  {
public:
  explicit HCalRechits(const edm::ParameterSet&);
  ~HCalRechits();
...
//
HCalRechits::HCalRechits(const edm::ParameterSet& iConfig) :
...
void
HCalRechits::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  using namespace edm;
//  using mynamespace::HCalRechits;
  n_pho = 0;
  pho_eb.clear();
  pho_ee.clear();
  pho_gap_eb_ee.clear();
              ...
              if(geo->getGeometry(hcalrh.id()) !=nullptr){
                ...
                diffFromSCPhi = deltaPhi(pho_sc_phi[n_pho],rechitPhi_new);
                  ...
                  if( 1.25663 < fabs(diffFromSCPhi) && fabs(diffFromSCPhi) < 1.8849)
                     {  std::cout<<" In Loop Photon Phi "<<pho_sc_phi[n_pho]<<" Rechit Phi "<<rechitPhi_new<<" Differnce "<<diffFromSCPhi<<std::endl;
...
int HCalRechits::calDIPhi(int iPhi1, int iPhi2) {
...
int HCalRechits::calDIEta(int iEta1, int iEta2) {
...
float HCalRechits::getMinEnergyHCAL(HcalDetId id) const {
...
void HCalRechits::reallocate_setaddress(int n_pho_, int n_hcalhit_)
{
...
void
HCalRechits::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
